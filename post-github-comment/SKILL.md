...
5. **Draft the comment** — Write the full comment text. If labels will be added/changed, note this alongside the draft.
6. **Post the comment and update labels** — Post via `gh issue comment` and apply labels via `gh issue edit`.
7. **Edit existing comments** — When updating a previously posted comment, ALWAYS use `gh api -X PATCH` to edit in place. NEVER delete and re-create comments. **Before editing, verify BOTH guards below — if either fails, do NOT edit; post a new comment instead.**

   1. **Author guard** — Fetch the comment's author (`.user.login`) and confirm it matches the currently authenticated account (`gh api user --jq .login`). **NEVER edit a comment created by a different user** — only edit comments you (the authenticated account) authored.
   2. **AI-attribution guard** — Confirm the comment body contains the `🤖 Comment generated by AI` footer. **NEVER edit a human-written comment**, even one authored by the same account.
...
- **Never edit another user's comment:** Only edit comments authored by the currently authenticated account AND carrying the AI footer — verify both before any `PATCH` (see Workflow step 7)
