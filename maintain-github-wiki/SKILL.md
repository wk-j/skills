...
description: Maintain GitHub repository Wiki pages safely in natural Thai while preserving technical terms in English, using Open Knowledge Format-compatible markdown, categorizing important repository knowledge, auditing existing wiki content, drafting page changes, and publishing verified updates automatically. Use when asked to create, update, sync, reorganize, or audit a GitHub Wiki for a repository; do not use for README/docs edits in the main repo.
---
...
Keep technical terms in their established English form — never translate them into Thai, transliterate them with Thai characters, invent Thai equivalents, or append parenthetical Thai translations. This covers code identifiers, file paths, API names, commands, package names, protocol terms, framework/library names, and established software vocabulary such as deploy, endpoint, cache, middleware, branch, commit, pull request, queue, worker, and token. Write the surrounding sentence in natural Thai and insert each technical term unchanged (e.g. "ระบบจะ retry งานที่ fail ผ่าน queue เดิม"). Translate only genuinely non-technical words.

When readers may not know a technical term, keep the term unchanged and explain what it does or why it matters in a separate Thai phrase or sentence. Do not present the explanation as a Thai replacement for the term. For example, write "middleware จะตรวจ request ก่อนส่งต่อไปยัง handler หลัก" rather than "middleware (ตัวกลาง)". If specialized terms recur across many pages, a `Glossary` page may explain their roles while retaining the original English terms as entry names.
