...
7. **Verify before publishing.** Inspect the diff, run markdown/metadata checks when available, confirm no page starts with a bare `---` frontmatter block, confirm no cross-page wiki link carries a `.md` suffix (those break to raw file views on GitHub), and scan for secrets or private data. If a requested update would require deleting pages, renaming pages, publishing secrets, or overwriting broad human-authored content without explicit instruction in the original request, stop and report the blocker instead of guessing.
